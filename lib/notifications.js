...
// Asks for permission and, outside Expo Go, returns an Expo push token
// that can be sent to Expo's push API from a server. Inside Expo Go this
// returns null (remote push isn't supported there since SDK 53) — local
// notifications still work outside Android+Expo Go.
...
// Silently asks for permission and saves the push token for a just-signed-in
// user. Meant to be fired from onAuthStateChange('SIGNED_IN') so both the
// partner and customer portals prompt for consent right after login,
// without requiring a trip to account settings. Never throws — a denied
// prompt or an Expo Go limitation just means no token gets saved.
